...
@Component("ebiMetagenomicsKeyResolver")
public class KeyResolverServiceImpl implements KeyResolverService {
    ...
    public eu.eoscpilot.schema2jsonld.web.common.Key resolve(String key) {
        ...
        if(!EbiMetagenomicsKey.KeyType.isValid(parts[0])) throw new ValidationException(String.format("unsupported type in key: %s", key));

        return new EbiMetagenomicsKey(EbiMetagenomicsKey.KeyType.from(parts[0]), parts[1]);
    }
}
